...
(select  ROW_NUMBER() OVER (order by code) row_number, 1 as type, date,code, volume, adjusted_close, previous_adjusted_close, adjusted_close - previous_adjusted_close last_move
from (
    ...
    where rank_number = 1 or rank_number = 2 order by code, rank_number desc
    )
...
UNION ALL
(select  ROW_NUMBER() OVER (order by code) row_number, 5 as type, date,code, volume, adjusted_close, previous_adjusted_close, adjusted_close - previous_adjusted_close last_move
from (
    select *, RANK() OVER ( partition by code
    order by date desc) rank from (
    with cte AS (
...
where rank = 1
order by code, date desc) order by date, code;
